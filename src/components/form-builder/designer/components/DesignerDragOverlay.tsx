...
function DesignerDragOverlay() {
    ...
    const isSidebarButton =
        draggedControl.data?.current?.isDesignerSidebarButton
    if (isSidebarButton) {
        const type = draggedControl.data?.current?.type as ControlType
        node = (
            <DesignerSidebarButtonDragOverlay
                formComponent={FormComponents[type]}
            />
        )
    }
    ...
    if (isDesignerComponentWrapper) {
        ...
        if (!control) {
            ...
            const DesignerComponent =
                FormComponents[control.type].designerComponent
            node = (
